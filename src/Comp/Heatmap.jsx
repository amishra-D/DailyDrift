...
const Heatmap = ({ user }) => {
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      setIsLoading(true);
      try {
        const data = await fetchCalendarData(user.uid);
  ...
  const transformMonthLabels = (months) => {
        ...
        {isLoading ? (
          ...
          <div className="overflow-x-auto pb-2">
            <CalendarHeatmap
              ...
              transformMonthLabels={(months) => months.map(month => ({
                ...month,
                y: month.y + 5,
              }))}
              transformWeekdayLabels={(weekdays) => weekdays.map(weekday => ({
                ...weekday,
                x: weekday.x + 5,
              }))}
              gutterSize={4}
